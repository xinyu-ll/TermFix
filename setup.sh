...
if command -v git >/dev/null 2>&1; then
  INSTALL_VERSION="$(git -C "$ROOT_DIR" rev-parse --short HEAD 2>/dev/null || echo unknown)"
fi
...
for scripts_dir in "${targets[@]}"; do
  ...
  cat > "$scripts_dir/termfixlib/_installed.py" <<EOF
"""Install metadata generated by setup.sh."""
...
echo "Done. Installed TermFix version: $INSTALL_VERSION"
echo "Important: setup.sh only copies files; it does not stop a running TermFix daemon."
echo "Restart iTerm2, or stop the existing termfix.py script and run Scripts -> AutoLaunch -> termfix.py."
