...
iTerm2 插件，自动捕获命令失败，通过 OpenAI 兼容 API 分析错误原因，在状态栏弹窗中展示修复建议。
...
![Cmd+J 错误分析弹窗](assets/cmdj.png)

按 `Cmd+L` 可以打开手动提问弹窗，基于当前终端上下文继续多轮对话，并从左侧恢复历史对话。

![Cmd+L 手动提问弹窗](assets/cmdl.png)
...
Control加鼠标左键点击状态栏中的 TermFix 组件，选择 **Configure**（或在 Status Bar 设置中双击组件），填写以下 knob：

| Knob | 说明 | 默认值 |
|------|------|--------|
| **Base URL** | OpenAI 兼容接口地址 | `https://api.openai.com` |
| **API Key** | 提供商 API Key | 空，**必填** |
| **Model** | 使用的模型 | `model_name` |
