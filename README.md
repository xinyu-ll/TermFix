...
```bash
cp termfix.py ~/Library/Application\ Support/iTerm2/Scripts/AutoLaunch/
cp -R termfixlib ~/Library/Application\ Support/iTerm2/Scripts/
```
...
5. 弹窗关闭后结果不会被清空。你可以再次按 `Cmd+J` 或点击 `TermFix` 重新打开同一条结果
